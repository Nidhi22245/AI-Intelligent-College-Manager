...
class ChatBot:
    def __init__(self,root):
        ...
        main_frame = Frame(self.root,bd=1,bg='#FCFCFC',width = 620)
        main_frame.pack()

        img_chat = Image.open("Images_GUI\chat.png")
        img_chat = img_chat.resize((190,80),Image.Resampling.LANCZOS)
        self.photoimg = ImageTk.PhotoImage(img_chat)

        Title_label = Label(main_frame,bd=1,relief = RAISED,anchor = 'nw', width=740,compound=LEFT,image=self.photoimg,text='Your Friendly Virtual Assistant: Simplifying Life Through Conversation!',font=('Microsof YaHei UI Light',11),fg='#104e8b',bg='white')
        Title_label.pack(side=TOP)

        self.scroll_y = ttk.Scrollbar(main_frame,orient=VERTICAL)
        self.text = Text(main_frame,width=65,height=20,bd=1 ,relief=RAISED,font=('Microsof YaHei UI Light',14),yscrollcommand=self.scroll_y.set)
        self.scroll_y.pack(side = RIGHT,fill=Y)
        self.text.pack()

        btn_frame = Frame(self.root,bd=1,bg='white',width = 620)
        btn_frame.pack()

        label_1=Label(btn_frame,text="Connect :",font=("Microsof YaHei UI Light",13),fg='#104e8b',bg='white')
        label_1.grid(row=0,column=0,padx=5,sticky=W)

        self.entry = StringVar()
        self.entry1 = ttk.Entry(btn_frame,textvariable=self.entry,width=40,font=('Microsof YaHei UI Light',14,'bold'))
        self.entry1.grid(row=0,column=1,padx=5,sticky=W)

        self.send=Button(btn_frame,text="Send",command=self.send,font=("Microsof YaHei UI Light",11),width=8,bg="#104e8b",fg="#FCFCFC")
        self.send.grid(row=0,column=2,padx=5,sticky=W)

        self.clear=Button(btn_frame,text="Clear Data",command=self.clear,font=("Microsof YaHei UI Light",11),width=8,bg="#104e8b",fg="#FCFCFC")
        self.clear.grid(row=0,column=3,padx=5,sticky=W)

        self.msg = ''
        self.label_2=Label(btn_frame,text=self.msg,font=('Microsof YaHei UI Light',14,'bold'),fg="#104e8b",bg="#FCFCFC")
